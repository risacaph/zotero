...
describe.skip("Trellis.CiteprocRs", function () {
	var chicagoNoteStyleID = "http://www.trellis.org/styles/chicago-shortened-notes-bibliography";
	var chicagoAuthorDateStyleID = "http://www.trellis.org/styles/chicago-author-date";
	var style;
	function getCiteprocJSEngine(style) {
		Trellis.Prefs.set('cite.useCiteprocRs', false);
		return style.getCiteProc('en-US', 'text');
	}
	function getCiteprocRSEngine(style) {
		Trellis.Prefs.set('cite.useCiteprocRs', true);
		return style.getCiteProc('en-US', 'text');
	}
	function createCitationItem(item) {
		return {
			id: item.id,
			uris: Trellis.Utilities.randomString(10),
			itemData: Trellis.Cite.System.prototype.retrieveItem(item.id)
		};
	}
	function createACitation(items) {
		if (!Array.isArray(items)) items = [items];
		return {
			citationID: Trellis.Utilities.randomString(10),
	...
	function assertProducedCitationsAreEqual(citation) {
		...
		let CRSBibl = Trellis.Cite.makeFormattedBibliography(citeprocRS, 'text');
		...
		let CJSBibl = Trellis.Cite.makeFormattedBibliography(citeprocJS, 'text');
	...
	before(async function () {
		...
		item1.libraryID = Trellis.Libraries.userLibraryID;
	...
	describe('with chicago-shortened-notes-bibliography.csl', function () {
		before(function () {
			style = Trellis.Styles.get(chicagoNoteStyleID);
		});
	...
	describe('with chicago-author-date.csl', function () {
		before(function () {
			style = Trellis.Styles.get(chicagoAuthorDateStyleID);
		});
