describe("Document Recognition", function () {
	...
	async function waitForProgressWindow() {
		...
		let progressWindow = getWindows("chrome://trellis/content/progressQueueDialog.xhtml")[0];
		let completeStr = Trellis.getString("general.finished");
	...
	before(function* () {
		...
		this.timeout(60000);
		Trellis.Prefs.set('autoRenameFiles.onMetadataChange', false); // Prevent auto-rename triggering during recognition
		// Load Trellis pane and install PDF tools
		yield Promise.all([
			loadTrellisPane().then(w => win = w)
		]);
	});
	...
	afterEach(async function () {
		...
		while (queue.getRows().some(row => row.status == Trellis.ProgressQueue.ROW_PROCESSING)) {
			await Trellis.Promise.delay(50);
		}
		
		queue.cancel();
		Trellis.RecognizeDocument._recognize.restore && Trellis.RecognizeDocument._recognize.restore();
	...
	describe("PDFs", function () {
		it("should recognize a PDF by DOI and rename the file", async function () {
			if (Trellis.automatedTest) this.skip(); // TODO: Mock services
			this.timeout(30000);
			// Import the PDF
			var testdir = getTestDataDirectory();
			testdir.append("recognizePDF_test_DOI.pdf");
			var attachment = await Trellis.Attachments.importFromFile({ file: testdir });
			
			win.TrellisPane.recognizeSelected();
			
			var addedIDs = await waitForItemEvent("add");
			var modifiedIDs = await waitForItemEvent("modify");
			assert.lengthOf(addedIDs, 1);
			var item = Trellis.Items.get(addedIDs[0]);
			...
			assert.equal(
				...
				Trellis.Attachments.getFileBaseNameFromItem(item) + '.pdf'
			);
			
			// The title should have changed
			assert.equal(
				attachment.getField('title'),
				Trellis.getString('file-type-pdf')
			);
		});
		...
		it("should rename a linked file attachment using parent metadata if no existing file attachments and pref enabled", async function () {
			...
			sinon.stub(Trellis.RecognizeDocument, "_recognize").callsFake(() => {
		...
		it("should use mime-type-derived extension for files with invalid extensions", async function () {
			...
			var attachment = await Trellis.Attachments.importFromFile({ file: tmpFileToImport });
	...
	describe("Ebooks", function () {
		it("should recognize an EPUB by ISBN and rename the file", async function () {
			...
			let search;
			let translateStub = sinon.stub(Trellis.Translate.Search.prototype, 'translate')
			...
			let attachment = await Trellis.Attachments.importFromFile({ file: testDir });
			...
			assert.lengthOf(addedIDs, 1);
			let item = Trellis.Items.get(addedIDs[0]);
			assert.equal(item.getField('title'), 'The Mania of the Nations on the Planet Mars: ISBN Database Edition');
			assert.equal(Trellis.Utilities.cleanISBN(item.getField('ISBN')), isbn);
			assert.lengthOf(modifiedIDs, 2);
			...
			assert.equal(
				attachment.attachmentFilename,
				Trellis.Attachments.getFileBaseNameFromItem(item) + '.epub'
			);

			// The title should have changed
			assert.equal(
				attachment.getField('title'),
				Trellis.getString('file-type-ebook')
			);
		...
		it("should recognize an EPUB by DOI and rename the file", async function () {
			...
			assert.equal(Trellis.Utilities.cleanDOI(item.getField('DOI')), doi);
			assert.lengthOf(modifiedIDs, 2);
		...
		it("should use metadata from EPUB when search returns item with different ISBN", async function () {
			...
			let testDir = getTestDataDirectory();
			testDir.append('recognizeEPUB_test_ISBN.epub');
			await Trellis.Attachments.importFromFile({ file: testDir });
	...
	describe("canUnrecognize()", function () {
		...
		async function getRecognizedItem() {
			...
			translateStub.restore();
			return Trellis.Items.get(addedIDs[0]);
		}
		...
		it("should return true for a recognized item with one attachment", async function () {
			...
			assert.isTrue(Trellis.RecognizeDocument.canUnrecognize(item));
		});
		...
		it("should return false for a recognized item with one trashed attachment", async function () {
			...
			assert.equal(item.numAttachments(), 1);
			assert.equal(item.numNotes(), 0);
			let attachment = Trellis.Items.get(item.getAttachments()[0]);
			attachment.deleted = true;
			await attachment.saveTx();
			assert.equal(item.numAttachments(), 0);
			assert.equal(item.numNotes(), 0);
			assert.isFalse(Trellis.RecognizeDocument.canUnrecognize(item));
		});
		...
		it("should return false for a recognized item with one attachment and a modified note", async function () {
			...
			await Trellis.Promise.delay(1200);
