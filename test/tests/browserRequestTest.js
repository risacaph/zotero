...
describe("Trellis.BrowserRequest", function () {
	describe("#downloadPDF()", function () {
		...
		it("should download a PDF from a JS redirect page", async function () {
			...
			httpd.registerFile(
				...
				Trellis.File.pathToFile(PathUtils.join(dir, 'test-pdf-redirect.html'))
			);
			httpd.registerFile(
				'/test.pdf',
				Trellis.File.pathToFile(PathUtils.join(dir, 'test.pdf'))
			);

			await Trellis.BrowserRequest.downloadPDF(`${baseURL}test-pdf-redirect.html`, tmpFile.path);

			var sample = await Trellis.File.getContentsAsync(tmpFile, null, 1000);
			assert.equal(Trellis.MIME.sniffForMIMEType(sample), 'application/pdf');
		});
		...
		it.skip("should display a viewer to clear a captcha if detected", async function () {
			...
			Trellis.Prefs.set('browserRequest.timeout', 10);
			let downloadPDFStub = sinon.stub(Trellis.BrowserRequest, "downloadPDFViaViewer");

			let promise = Trellis.BrowserRequest.downloadPDF('https://trellis-static.s3.amazonaws.com/test-pdf-redirect.html', tmpFile.path,
				{ shouldDisplayCaptcha: true });
			await new Promise(resolve => downloadPDFStub.callsFake((...args) => {
				resolve();
				Trellis.Prefs.set('browserRequest.timeout', 60e3);
				return downloadPDFStub.wrappedMethod.call(Trellis.BrowserRequest, ...args);
			}));
	...
	describe("#getEntryForURL()", function () {
		it("returns null for unrecognized URLs", function () {
			assert.isNull(Trellis.BrowserRequest.getEntryForURL('https://example.com/foo'));
		});

		it("matches registered substrings", function () {
			let entry = Trellis.BrowserRequest.getEntryForURL(
	...
	describe("#_pollForCookie()", function () {
		it("ignores a stale cookie present at poll start", async function () {
			...
			let readStub = sinon.stub(Trellis.BrowserRequest, "_readCookieValue");
			readStub.returns("stale-value");

			let onFound = sinon.spy();
			let handle = Trellis.BrowserRequest._pollForCookie({
			...
			try {
				await Trellis.Promise.delay(40);
	...
	describe("translator request retry", function () {
		function makeUtils() {
			...
			return new Trellis.Utilities.Translate(fakeTranslate);
		}
		...
		it("retries via clearChallenge when the initial request throws a registered 403", async function () {
			...
			let clearStub = sinon.stub(Trellis.BrowserRequest, "clearChallenge").resolves();
			let call = 0;
			let requestStub = sinon.stub(Trellis.HTTP, "request").callsFake(() => {
				call++;
				if (call === 1) {
					let xhr = {
						status: 403,
					...
					throw new Trellis.HTTP.UnexpectedStatusException(xhr, url, "Forbidden");
				}
				return {
					status: 200,
					response: { ok: true },
		...
		it("does not retry when the URL is not registered", async function () {
			...
			try {
				...
				assert.instanceOf(err, Trellis.HTTP.UnexpectedStatusException);
		...
		it("does not retry in browser on a 200 body that matches detectBlock (status must also match)", async function () {
			...
			let clearStub = sinon.stub(Trellis.BrowserRequest, "clearChallenge").resolves();
			let requestStub = sinon.stub(Trellis.HTTP, "request").returns({
				status: 200,
				response: '{"turnstile_required":true}',
				responseText: '{"turnstile_required":true}',
				getAllResponseHeaders: () => ''
