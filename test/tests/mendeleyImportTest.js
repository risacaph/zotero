/* global setHTTPResponse:false, sinon: false, Trellis_Import_Mendeley: false, HttpServer: false, createAnnotation: false */

describe('Trellis_Import_Mendeley', function () {
	var { Trellis_Import_Mendeley } = ChromeUtils.importESModule("chrome://trellis/content/import/mendeley/mendeleyImport.mjs");
	var server, httpd, httpdURL, importers;

	const getImporter = ({ importNotes = false } = {}) => {
		const importer = new Trellis_Import_Mendeley();
	...
	before(async () => {
		...
		httpd.registerFile(
			'/file1.pdf',
			Trellis.File.pathToFile(OS.Path.join(getTestDataDirectory().path, 'recognizePDF_test_title.pdf'))
		);
	});

	after(async () => {
		await new Trellis.Promise(resolve => httpd.stop(resolve));
	});
	...
	beforeEach(async () => {
		...
		setHTTPResponse(server, 'https://api.mendeley.com/', {
			...
			headers: {},
			json: JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/folders-simple.json')
			)
		});
		...
		setHTTPResponse(server, 'https://api.mendeley.com/', {
			...
			headers: {},
			json: JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/annotations.json')
			)
		});
		...
		setHTTPResponse(server, 'https://api.mendeley.com/', {
			...
			headers: {},
			json: JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/items-simple.json')
			)
		});
		...
		setHTTPResponse(server, 'https://api.mendeley.com/', {
			...
			headers: {},
			json: JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/user.json')
			)
		});
		...
		setHTTPResponse(server, 'https://api.mendeley.com/', {
			...
			headers: {},
			json: JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/groups.json')
			)
		});
		...
		setHTTPResponse(server, 'https://api.mendeley.com/', {
			...
			status: 200,
			json: JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/group-annotations.json')
			)
		});
	...
	afterEach(async () => {
		await Promise.all(
			importers
				.map(importer => ([
					Trellis.Items.erase(Array.from(new Set(importer.newItems)).map(i => i.id)),
					Trellis.Collections.erase(Array.from(new Set(importer.newCollections)).map(c => c.id))
				]))
	...
	describe('#import', () => {
		it("should import collections, items, attachments & annotations", async () => {
			...
			assert.equal(journal.itemTypeID, Trellis.ItemTypes.getID('journalArticle'));
			...
			assert.equal(report.itemTypeID, Trellis.ItemTypes.getID('report'));
			...
			assert.equal(withpdf.itemTypeID, Trellis.ItemTypes.getID('journalArticle'));
			...
			// collection
			const parentCollection = await Trellis.Collections.getAsync(
		...
		it("should update previously imported item, based on config", async () => {
			...
			setHTTPResponse(server, 'https://api.mendeley.com/', {
				...
				headers: {},
				json: JSON.parse(
					await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/items-updated.json')
				)
			});
			...
			assert.equal(report.itemTypeID, Trellis.ItemTypes.getID('journalArticle'));
			assert.equal(report.getField('year'), '2002');
		...
		it("shouldn't update previously imported item, based on config", async () => {
			...
			const noNewItemHere = await Trellis.Relations.getByPredicateAndObject('item', 'mendeleyDB:documentUUID', '86e56a00-5ae5-4fe8-a977-9298a03b16d6');
			...
			const newItem = (await Trellis.Relations
		...
		it("should correct IDs if available on subsequent import", async () => {
			setHTTPResponse(server, 'https://api.mendeley.com/', {
				...
				headers: {},
				json: JSON.parse(
					await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/items-simple-no-desktop-id.json')
				)
			});
			const importer = getImporter();
		...
		it("should handle empty creators and tags", async () => {
			setHTTPResponse(server, 'https://api.mendeley.com/', {
				...
				headers: {},
				json: JSON.parse(
					await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/items-bad-data.json')
				)
			});
			...
			const journalNoAuthors = (await Trellis.Relations
			...
			const journalEmptyAuthors = (await Trellis.Relations
			...
			const journalEmptyTags = (await Trellis.Relations
		...
		it('should translate a notebook content into Trellis note content', async () => {
			...
			let mendeleyNotebook = JSON.parse(
				await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/notebook.json')
			);
			const importer = getImporter();
			const noteContent = await importer._translateNotebookToNoteContent(Trellis.Libraries.userLibraryID, mendeleyNotebook);
		...
		it('should import a notebook from Mendeley', async () => {
			...
			setHTTPResponse(server, 'https://api.mendeley.com/', {
				...
				headers: {},
				json: JSON.parse(
					await Trellis.File.getContentsFromURLAsync('resource://trellis-unit-tests/data/mendeleyMock/notebooks.json')
				)
			});
			...
			const note = (await Trellis.Relations
		...
		it('should handle incorrect annotations', async () => {
			...
			assert.lengthOf(await Trellis.Relations.getByPredicateAndObject('item', 'mendeleyDB:annotationUUID', '885615a7-170e-4613-af80-0227ea76ae55'), 0);
			assert.lengthOf(await Trellis.Relations.getByPredicateAndObject('item', 'mendeleyDB:annotationUUID', 'invalid'), 0);
			// Colorless note annotations should have a default colour.
			const colorlessNote = (await Trellis.Relations
				.getByPredicateAndObject('item', 'mendeleyDB:annotationUUID', 'bfbdb972-171d-4b21-8ae6-f156ac9a2b41'))
				.filter(item => item.libraryID == Trellis.Libraries.userLibraryID && !item.deleted)
				.shift();
