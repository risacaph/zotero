describe("Trellis.Date", function () {
	describe("#getMonths()", function () {
		...
		before(function () {
			origLocale = Trellis.locale;
		});
		
		after(function () {
			Trellis.locale = origLocale;
		});
		...
		describe("French", function () {
			...
			it("should get French long months with English", function () {
				let months = Trellis.Date.getMonths(true).long;
				assert.lengthOf(months, 24);
	...
	describe("#sqlToDate()", function () {
		it("should convert an SQL local date into a JS Date object", async function () {
			...
			var sqlDate = d1.getFullYear()
				+ '-'
				+ Trellis.Utilities.lpad(d1.getMonth() + 1, '0', 2)
				+ '-'
				+ Trellis.Utilities.lpad(d1.getDate(), '0', 2)
				+ ' '
				+ Trellis.Utilities.lpad(d1.getHours(), '0', 2)
				+ ':'
				+ Trellis.Utilities.lpad(d1.getMinutes(), '0', 2)
				+ ':'
				+ Trellis.Utilities.lpad(d1.getSeconds(), '0', 2);
			var offset = d1.getTimezoneOffset() * 60 * 1000;
			var d2 = Trellis.Date.sqlToDate(sqlDate);
			assert.equal(
				Trellis.Date.sqlToDate(sqlDate).getTime(),
		...
		it("should convert an SQL UTC date into a JS Date object", async function () {
			...
			date = Trellis.Date.sqlToDate(date, true);
	...
	describe("#isISODate()", function () {
		it("should determine whether a date is an ISO 8601 date", function () {
			assert.ok(Trellis.Date.isISODate("2015"));
			assert.ok(Trellis.Date.isISODate("2015-04"));
			assert.ok(Trellis.Date.isISODate("2015-04-29"));
			assert.ok(Trellis.Date.isISODate("2015-04-29T17:28"));
			assert.ok(Trellis.Date.isISODate("2015-04-29T17:28Z"));
			assert.isFalse(Trellis.Date.isISODate("2015-04-29 17:28"));
		})
	})
	...
	describe("#strToDate()", function () {
		...
		it("should parse English month before date", function () {
			var o = Trellis.Date.strToDate("June 26, 2010");
		...
		it("should parse English month after date", function () {
			var o = Trellis.Date.strToDate("26 June 2010");
			assert.equal(o.month, 5);
			assert.equal(o.day, 26);
			assert.equal(o.year, 2010);
		});
		...
		it("should parse two-digit month with leading zero and four-digit year", function () {
			var o = Trellis.Date.strToDate('08/2020');
			assert.equal(o.month, 7);
			assert.isUndefined(o.day);
			assert.equal(o.year, 2020);
		});
		...
		it("should parse string with just month number", function () {
			var o = Trellis.Date.strToDate('1');
		...
		it("should parse string with just day number", function () {
			var o = Trellis.Date.strToDate('25');
	...
	describe("#isHTTPDate()", function () {
		it("should determine whether a date is an RFC 2822 compliant date", function () {
			assert.ok(Trellis.Date.isHTTPDate("Mon, 13 Jun 2016 02:09:08   +4000"));
			assert.ok(Trellis.Date.isHTTPDate("13 Jun 2016 02:09:08 +4000"));
			assert.ok(Trellis.Date.isHTTPDate("13 Jun 2016   02:09 +4000"));
			assert.ok(Trellis.Date.isHTTPDate("13  Jun  2016 02:09 EDT"));
		})
	})
})
