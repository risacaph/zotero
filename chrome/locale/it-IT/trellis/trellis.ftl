...
import-source-file =
    .label = File (BibTeX, RIS, Trellis RDF, ecc.)
...
import-mendeley-encrypted = Non è possibile leggere il database Mendeley selezionato, probabilmente perché crittografato. Leggi <a data-l10n-name="mendeley-import-kb">Come importare una biblioteca Mendeley in Trellis?</a> per ulteriori informazioni.
...
publications-sharing-text = Puoi decidere di riservare tutti i diritti della tua opera, di pubblicarla sotto licenza Creative Commons oppure di assegnarla al pubblico dominio. In ogni caso, tieni presente che la tua opera verrà messa a disposizione del pubblico tramite trellis.org.
publications-sharing-prompt = Vuoi permettere che la tua opera sia condivisibile da altri?
publications-sharing-reserved =
    .label = No, pubblica la mia opera solo su trellis.org
...
account-not-logged-in-text = Accedi al tuo account Trellis per sincronizzare i dati.
...
mac-word-plugin-install-message = Trellis richiede l'accesso alla cartella dati di Word per installare il plugin.
