...
import-source-file =
    .label = Bir dosya (BibTeX, RIS, Trellis RDF, vb.)
...
publications-intro = Yayınlarım'a eklediğiniz eserler trellis.org adresindeki profil sayfanızda gösterilecektir. Eklenti dosyalarını dahil etmeyi seçerseniz, bu dosyalar belirttiğiniz lisans uyarınca halka açık şekilde paylaşılacaktır. Sadece kendi ürettiğiniz eserleri ekleyiniz ve sadece dağıtım hakkına sahip olduğunuz ve paylaşmak istediğiniz dosyaları dahil ediniz.
...
publications-sharing-text = Kendi eserlerinizin tüm haklarını saklı tutabilir, Creative Commons altında lisanslayabilir ya da kamu alanına açıkça sunabilirsiniz. Her durumda eser trellis.org aracılığıyla halka açık olacaktır.
publications-sharing-prompt = Eserinizin başkaları tarafından paylaşılabilmesine izin veriyor musunuz?
publications-sharing-reserved =
    .label = Hayır, eserimi sadece trellis.org adresinde yayınla
...
mac-word-plugin-install-message = Word eklentisini kurmak için, Trellis'nun Word verilerine erişmesi gerekiyor.
