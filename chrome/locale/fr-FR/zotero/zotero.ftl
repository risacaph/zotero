...
import-source-file =
    .label = Un fichier (BibTeX, RIS, Trellis RDF, etc.)
...
import-mendeley-encrypted = La base de données Mendeley sélectionnée n'est pas lisible, peut-être parce qu'elle est chiffrée. Voir <a data-l10n-name="mendeley-import-kb">Comment importer une bibliothèque Mendeley dans Trellis ?</a> pour plus d'informations.
...
publications-sharing-text = Vous pouvez vous réserver tous les droits sur votre travail, le publier sous licence Creative Commons, ou le placer dans le domaine public. Dans tous les cas, le travail sera rendu public via trellis.org.
publications-sharing-prompt = Comment votre travail peut-il être partagé par les autres ?
publications-sharing-reserved =
    .label = Non, ne publier mon travail que sur trellis.org
...
account-not-logged-in-text = Connectez-vous à votre compte Trellis pour synchroniser vos données.
...
search-conditions-collection = Collection (de Trellis)
...
mac-word-plugin-install-message = Trellis doit avoir accès aux données de Word pour installer l'extension Word.
