 ...
 * without big alterations to existing Trellis code
 */
Trellis.CiteprocRs = {
	init: async function () {
		if (Trellis.CiteprocRs.deferred) {
			return Trellis.CiteprocRs.deferred.promise;
		}
		Trellis.CiteprocRs.deferred = Trellis.Promise.defer();
		Trellis.debug("require('citeproc_rs_wasm')");
		// This is kind of nasty:
		// Due to Trellis being based on an old Firefox platform we are unable to use
		// the commonJS modules generated by citeproc_rs because they use too
		// advanced features.
		// Cormac has decided to provide a custom build just for Trellis
		// where citeproc_rs_wasm_include.js attaches some classes to Trellis.CiteprocRs.
		// This works fine until we call resetDB() in the test runner (or any other time we
		// could call TrellisContext.reinit(). A usual require() call returns exported symbols
		// from some Gecko script cache. Unfortunately that means that the necessary citeproc
		// classes do not get reattached to the Trellis object and the Citeproc Wasm driver breaks
		// when trying to create its own objects (e.g. Trellis.CiteprocRs.WasmResult).
		...
		// Trellis symbol so that things don't break. Obviously this isn't optimal.
		...
		Components.classes["@mozilla.org/moz/jssubscript-loader;1"]
			...
			.loadSubScript('resource://trellis/citeproc_rs_wasm_include.js',
				{ Trellis });
		// require('citeproc_rs_wasm_include');
		const CiteprocRs = require('citeproc_rs_wasm');
		// Initialize the wasm code
		Trellis.debug("Loading citeproc-rs wasm binary");
		const xhr = await Trellis.HTTP.request('GET', 'resource://trellis/citeproc_rs_wasm_bg.wasm', {
			responseType: "arraybuffer"
		});
		Trellis.debug("Initializing the CiteprocRs wasm driver");
		await CiteprocRs(xhr.response);
		Trellis.debug("CiteprocRs driver initialized successfully");
		this.Driver = CiteprocRs.Driver;
		Trellis.CiteprocRs.deferred.resolve();
	},
	...
	Engine: class {
		...
		free(ignoreErrors=false) {
			try {
				Trellis.debug('CiteprocRs: free Driver', 5);
				this._driver.free();
			}
			catch (e) {
		...
		_resetDriver() {
			...
			Trellis.debug('CiteprocRs: new Driver', 5);
			let options = {
			...
			this._driver = new Trellis.CiteprocRs.Driver(options);
		}
		
		async _fetchLocale(lang) {
			return Trellis.Cite.System.prototype.retrieveLocale(lang);
		}
		...
		_insertCitationReferences(citation) {
			...
			for (const citationItem of citation.citationItems) {
				...
				Trellis.debug(`CiteprocRs: insertReference ${JSON.stringify(citeprocItem)}`, 5);
		...
		previewCitationCluster(citation, citationsPre, citationsPost, outputFormat) {
			if (!citation.citationID) citation.citationID = Trellis.Utilities.randomString(10);
			...
			Trellis.debug(`CiteprocRs: previewCluster ${JSON.stringify([cluster, allClusterOrder, outputFormat])}`, 5);
			return this._driver.previewCluster(cluster, allClusterOrder, outputFormat);
		}
		
		// This is an undocumented citeproc-js endpoint that used to be used by Trellis
		// when generating citations for clipboard or export.
		appendCitationCluster(citation) {
			this.insertCluster(citation);
			Trellis.debug(`CiteprocRs: builtCluster ${citation.citationID}`, 5);
		...
		insertCluster(citation) {
			...
			Trellis.debug(`CiteprocRs: insertCluster ${JSON.stringify(cluster)}`, 5);
		...
		setClusterOrder(citations) {
			...
			Trellis.debug(`CiteprocRs: setClusterOrder ${JSON.stringify(clusters)}`, 5);
			this._driver.setClusterOrder(clusters);
		}
		
		getBatchedUpdates() {
			Trellis.debug(`CiteprocRs: batchedUpdates`, 5);
		...
		updateUncitedItems(itemIDs) {
			...
			Trellis.debug(`CiteprocRs: includeUncitedItems ${JSON.stringify(referenceIDs)}`);
			this._driver.includeUncited({ Specific: referenceIDs });
		}
		
		updateItems(itemIDs) {
			Trellis.debug('CiteprocRstoJs: updateItems [forwarding to updateUncitedItems()]');
			return this.updateUncitedItems(itemIDs);
		}
		
		makeBibliography() {
			Trellis.debug(`CiteprocRs: bibliographyMeta`, 5);
			...
			Trellis.debug(`CiteprocRs: makeBibliography`, 5);
