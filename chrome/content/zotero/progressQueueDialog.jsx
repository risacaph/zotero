...
function _init() {
	...
	document.title = Trellis.getString(_progressQueue.getTitle());
...
/**
 * Focus items in Trellis library when double-clicking them in the Retrieve
 * metadata window.
 * @param {Event} event
...
async function _handleActivate(event, indices) {
	if (event && event.type === 'dblclick') {
		...
		if (win) {
			win.TrellisPane.selectItem(itemID);
			win.focus();
		}
	}
}
